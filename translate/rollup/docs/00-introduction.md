...
Rollup是一个将小型代码打包为大型代码或应用的JavaScript模块打包器。
...
Rollup能够通过插件引入使用CommonJS模块化的代码。
...
使用Rollup将ES模块代码打包为UMD模块代码或者CommonJS模块代码，以保证你的ES模块代码能够同时在node.js和webpack中使用。然后在package.json文件中通过main字段指定打包好的版本。如果你的package.json有指定module字段的话，Rollup和webpack2等es模块打包工具会直接引入ES模块版本。