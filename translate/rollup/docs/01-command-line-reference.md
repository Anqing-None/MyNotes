...
Rollup通常是在命令行中使用。你也可以指定一个配置文件以简化命令行使用步骤并设置一些高级特性。
...
Rollup配置文件是可选的，但是它很强大且便捷，因此推荐使用。一个配置文件的本质是使用ES模块导出的对象，对象中包含一些配置信息。
...
如果你想用CommonJS模块语法来写Rollup配置文件，那么你应该将该文件命名为rollup.config.cjs。
...
你也可以使用TypeScript来编写配置文件，安装插件即可。
...
以下列出了配置文件支持的选项。每个选项的细节信息可以查看[big list of options](guide/en/#big-list-of-options)。
...
命令行配置参数会优先于配置文件中的配置参数。如果你想让文件配置参数优先，你需要在配置文件中接收命令行参数，然后手动忽略它们。
